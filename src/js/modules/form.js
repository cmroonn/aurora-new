...
export default class Form {
    ...
    this.$form.addEventListener(
      ...
      (e) => {
        if (e.target.classList.contains("validate", "error")) {
          ...
          if (!document.querySelectorAll(".error", this.$form).length) {
  ...
  validateForm() {
    document.querySelectorAll(".validate", this.$form).forEach((input) => {
  ...
  setDisableBtn() {
    document
      .querySelectorAll("[data-submit]", this.$form)
  ...
  setError(input, err) {
    const errEl = input.closest(".form__input-wrapper")
